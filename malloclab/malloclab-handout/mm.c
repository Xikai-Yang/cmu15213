...
void *cal_list_header(size_t size) {
    ...
    return heap_start + index*DSIZE*2 + WSIZE;
...
void print_heap() {
    char * ptr = mem_heap_lo() + WSIZE + DSIZE*2*LEN;
...
void *first_fit(size_t alloc_size)
{
    ...
    */
    ...
    if (ptr != NULL) {
        ...
        int extend_size = MAX(alloc_size, CHUNKSIZE);
        void *ptr = extend_heap(extend_size);
        if (ptr == NULL) {
            return NULL;
        }
        return ptr;
...
{
    void *list_start = cal_list_header(alloc_size);
    void *bound = mem_heap_lo() + DSIZE*2*LEN;
    ...
    if (ptr == NULL) {
        int extend_size = MAX(alloc_size, CHUNKSIZE);
        //alloc_size = MAX(alloc_size, CHUNKSIZE);
        void *ptr = extend_heap(extend_size);
...
void *recoalesce(void *ptr, size_t size)
{
    ...
    void *next_header = footer + WSIZE;
    int next_alloc = GET_ALLOC(next_header);
    if (next_alloc && next_header == (mem_heap_hi() - WSIZE * 2)) {
        // try entending the heap
        
        size_t remainder = size - GET_SIZE(header);
        int extend_size = MAX(remainder, CHUNKSIZE);
        if (extend_heap(extend_size) == NULL) {
            return NULL;
        }
        remainder = extend_size - remainder;
        PUT(header, PACK(size, 1));
        PUT(footer, PACK(size, 1));
        PUT(footer + WSIZE, PACK(remainder, 0));
        PUT(footer + remainder, PACK(remainder, 0));
        return header;
