...
int input_error_handler(char **argv, int fg)
{
    if (argv[1] == NULL) {
        if (fg) {
            Sio_puts("fg");
...
void fg(char **argv)
{
    ...
    if (stopped_job->state == ST) {
        Kill((stopped_job->pid)*(-1), SIGCONT);
    }
    
    stopped_job->state = FG;
    
    waitfg(stopped_job->pid);
...
void do_bgfg(char **argv) 
{
    ...
    if (!strcmp(argv[0], "bg")) {
        // convert a bg stopped to a bg running
        bg(argv);
    }
...
void waitfg(pid_t pid)
{
    ...
    while (fgpid(jobs)) {
        ...
        Sio_puts("still continue is : ");
        Sio_putl(fgpid(jobs));
        Sio_puts("state is : ");
        Sio_putl(getjobpid(jobs, temp_pid)->state);
        Sio_puts("   ");
        Sio_puts(getjobpid(jobs, temp_pid)->cmdline);
        Sio_puts("\n");
        */
...
void sigtstp_handler(int sig) 
{
    ...
    getjobpid(jobs, pid)->state = ST;  // otherwise trace08 will fail
    Kill((pid) * (-1), sig);
    return;
}
